...
makeTrialTypeVar <- function(indata){
  ...
  # Only works if *ALL* observations are from the *SAME* program
  if(indata$programName[1]=="IITA"){
    outdata <- indata %>% mutate(
      TrialType=ifelse(
        grepl("CE|clonal|13NEXTgenC1", studyName, ignore.case=T),"CET",NA),
      TrialType=ifelse(
        grepl("EC",studyName,ignore.case = T),"ExpCET",TrialType),
      TrialType=ifelse(
        grepl("PYT",studyName,ignore.case = T),"PYT",TrialType),
      TrialType=ifelse(
        grepl("AYT",studyName,ignore.case = T),"AYT",TrialType),
      TrialType=ifelse(
        grepl("UYT",studyName,ignore.case = T),"UYT",TrialType),
      TrialType=ifelse(
        grepl("geneticgain|gg|genetic gain",studyName,ignore.case = T),"GeneticGain",TrialType),
      TrialType=ifelse(
        grepl("Cassava",studyName,ignore.case = T) & grepl("/",studyName),"GeneticGain",TrialType),
      TrialType=ifelse(
        (grepl("clonal evaluation trial",!grepl("genetic gain",studyDescription,ignore.case = T), ignore.case = T)),"CET",TrialType),
      TrialType=ifelse(
        grepl("preliminary yield trial",studyDescription,ignore.case = T),"PYT",TrialType),
      TrialType=ifelse(
        grepl("Crossingblock|\\.CB\\.|cross",studyName) & is.na(TrialType),"CrossingBlock",TrialType),
      TrialType=ifelse(
        grepl("NCRP",studyName) & is.na(TrialType),"NCRP",TrialType),
      TrialType=ifelse(
        grepl("conservation",studyName) & is.na(TrialType),"Conservation",TrialType),
      TrialType=ifelse(
        grepl("seedling|\\.SN",studyName),"SN",TrialType))
  }

  if(indata$programName[1]=="NRCRI"){
    outdata <- indata %>%
      mutate(TrialType=ifelse(grepl("TP1",studyName,ignore.case = T),"TP1",NA),
           ...
           TrialType=ifelse(
             grepl("15nextgen60gs-cbUM|crossnblk|crossingblock",studyName,ignore.case = T) & !grepl("CET",studyName),
             "CrossingBlock",TrialType),
           TrialType=ifelse(grepl("seedling",studyName,ignore.case = T),NA,TrialType))
  }

  if(indata$programName[1]=="TARI"){
    outdata <- indata %>%
      mutate(
        TrialType=ifelse(
          grepl("Advanced Yield|AYT", trialType, ignore.case = T),"AYT",NA),
        TrialType=ifelse(
          grepl("Clonal|CET", trialType, ignore.case = T),"CET",TrialType),
        TrialType=ifelse(
          grepl("Preliminary|PYT", trialType, ignore.case = T),"PYT",TrialType),
        TrialType=ifelse(
          grepl("Regional", trialType, ignore.case = T),"RegionalTrial",TrialType),
        TrialType=ifelse(
          grepl("Uniform|UYT", trialType, ignore.case = T),"UYT",TrialType),
        TrialType=ifelse(
          grepl("Variety Release", trialType, ignore.case = T),"VarietyTrial",TrialType),
        TrialType=ifelse(
          grepl("CROSSING", trialType, ignore.case = T),"CrossingBlock",TrialType),
        TrialType=ifelse(
          grepl("GWAS", trialType, ignore.case = T),"GWAS",TrialType))
  }
...
renameAndSelectCols <- function(traitabbrevs,indata,
  ...
  outdata <- indata %>%
    select(
      any_of(c("studyYear","programName","locationName","studyName",
               ...
               "plotNumber","rowNumber","colNumber","entryType","trialType",
               "plantsPerPlot","numberBlocks","numberReps")),
...
# All of the columns appear inside the nested data.frames
# The data.frames are labeled by trait
nestDesignsDetectedByTraits <- function(indata, traits){
  indata <- indata %>%
    ...
    filter(!is.na(Value), !is.na(GID)) %>%
    tidyr::nest(MultiTrialTraitData=c(-Trait))
  return(indata)
}
...
runCrossVal <- function(TrainTestData,modelType,grms,nrepeats,nfolds,ncores=1,
  ...
  fitModel <- possibly(function(splits,modelType,augmentTP,TrainTestData,GroupName,grms){
    ...
    if(modelType %in% c("AD","ADE")){
      ...
      if(modelType=="ADE"){
        gblups <- gblups %>% mutate(#GEEDaa=as.numeric(fit$U[[paste0("u:",gid,"aa")]]$drgBLUP),
    ...
    accuracy <- accuracy %>% mutate(computeTime=computeTime)
    return(accuracy)
  },otherwise = NA)
...
runGenomicPredictions <- function(blups,modelType,grms,ncores=1,gid="GID",...){
  ...
  runOnePred <- possibly(function(trainingdata,modelType,grms){
    ...
    pev <- tibble(GID=names(pev),
                PEVa=pev)
    gblups <- gblups %>% left_join(pev)
    if(modelType %in% c("AD","ADE")){
      gblups <- gblups %>% mutate(GEDD=as.numeric(fit$U[[paste0("u:",gid,"d")]]$drgBLUP))
      ...
      if(modelType=="ADE"){
        gblups <- gblups %>% mutate(GEEDad=as.numeric(fit$U[[paste0("u:",gid,"ad")]]$drgBLUP))
